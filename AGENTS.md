...
- A home tem cinco cenas: hero, índice dos sete pilares, Sobre mim, canais e newsletter. Os pilares têm páginas próprias em `/baleias`, `/instituicoes`, `/tecnica`, `/macro`, `/onchain`, `/ordinais` e `/derivativos`. A barra de letras leva a essas páginas. Preserve as definições das verticais em `lib/bitmood/chapters.ts` e o mapeamento de esculturas em `lib/bitmood/world-progress.js`.
