...
export function createAxisJourney({ root, track, count, pillarCount, onScene, onPillar, isBlocked }) {
  let scene = 0, pillar = 0, swipe = null, wheelAt = -Infinity, wheelTotal = 0, wheelAxis = '', wheelUsed = false, wheelCommitAt = -Infinity;
  ...
  function reading(target, direction = 1) {
    const copy = root.querySelector('.scene-copy.is-active');
    const overflow = copy && copy.scrollHeight > copy.clientHeight + 2 && target?.closest?.('.scene-copy.is-active') === copy;
    // At a pillar's text boundary the next gesture must be able to change pillar.
    const canRead = overflow && (direction < 0 ? copy.scrollTop > 1 : copy.scrollTop < copy.scrollHeight - copy.clientHeight - 1);
    return scene === 2 || (scene === 1 ? canRead : overflow);
  }
  function wheel(event) {
    ...
    if (axis === 'y' && reading(event.target, Math.sign(event.deltaY))) return;
    event.preventDefault();
    const now = performance.now();
    if (now - wheelAt > 140 || (wheelUsed && now - wheelCommitAt >= 360)) { wheelUsed = false; wheelTotal = 0; wheelAxis = axis; }
    wheelAt = now;
    ...
    if (Math.abs(wheelTotal) >= 30) { wheelUsed = true; wheelCommitAt = now; move(axis, Math.sign(wheelTotal)); }
  }
  function start(event) {
    swipe = null;
    if (isBlocked() || event.touches.length !== 1 || ignoresSceneKeys(event.target) || (window.visualViewport?.scale ?? 1) > 1) return;
    const touch = event.touches[0]; swipe = { x: touch.clientX, y: touch.clientY, axis: null, used: false, target: event.target, reading: false };
  }
  function touch(event) {
    ...
    if (!swipe.axis && (dx || dy)) { swipe.axis = Math.abs(dx) > Math.abs(dy) ? 'x' : 'y'; swipe.reading = reading(swipe.target, Math.sign(dy)); }
    if (swipe.axis === 'y' && swipe.reading) return;
    if (event.cancelable) event.preventDefault();
    const distance = swipe.axis === 'x' ? dx : dy;
    if (!swipe.used && Math.abs(distance) >= 30) { swipe.used = true; move(swipe.axis, Math.sign(distance)); }
  }
  ...
  function key(event) {
    ...
    if (vertical && reading(event.target, vertical)) return;
