...
/** Native wheel/snap navigation, with one-scene swipes for touch screens. */
export function createJourney({ root, track, stage, count, progress, getMotion, isBlocked, onScene }) {
  ...
  function touchStart(event) {
    touching = true; interrupt(); swipe = null;
    if (isBlocked() || event.touches?.length !== 1 || ignoresSceneKeys(event.target) || (window.visualViewport?.scale ?? 1) > 1) return;
    const touch = event.touches[0];
    swipe = { x: touch.clientX, y: touch.clientY, id: touch.identifier, target: event.target, owner: null, committed: false };
  }
  function touchMove(event) {
    if (!swipe) return;
    // Never take over a pinch, form interaction, or gesture already owned by the browser.
    if (isBlocked() || event.touches.length !== 1 || !event.cancelable) { swipe = null; return; }
    const touch = event.touches[0];
    if (touch.identifier !== swipe.id) { swipe = null; return; }
    const dx = touch.clientX - swipe.x, dy = swipe.y - touch.clientY;
    if (!swipe.owner) {
      if (dx === 0 && dy === 0) return;
      // Decide on the first move, before Chrome commits native viewport panning.
      swipe.owner = Math.abs(dx) >= Math.abs(dy) || canScrollCopy(swipe.target, Math.sign(dy)) ? 'native' : 'scene';
    }
    if (swipe.owner === 'native') return;
    event.preventDefault();
    // Short taps/jitter do not navigate; a held or reversed swipe cannot skip chapters.
    if (swipe.committed || Math.abs(dy) < 36) return;
    swipe.committed = true;
    animateTo(Math.round(target) + Math.sign(dy));
  }
  function touchEnd(event) {
    swipe = null; touching = event.touches.length > 0;
  ...
  return {
    ...
    dispose() {
      ...
      track.removeEventListener('touchstart', touchStart); track.removeEventListener('touchmove', touchMove); track.removeEventListener('touchend', touchEnd); track.removeEventListener('touchcancel', touchEnd);
