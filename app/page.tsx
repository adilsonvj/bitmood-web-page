  ...
  return <div ref={rootRef} className={`bitmood axis-journey ${ready?"is-ready":"is-loading"} ${graphicsFailed?"graphics-fallback":""} ${motionPaused?"motion-paused":""}`} data-scene={current.id}>
    ...
    <main ref={trackRef} className="journey" aria-label="O universo BITMOOD" aria-describedby="navigation-help">
      <div className="experience-stage" ref={stageRef}>
        ...
        {active===1&&<nav className="vertical-rail" aria-label="Escolher pilar"><button className="pillar-rail-arrow" aria-label="Pilar anterior" onClick={()=>journeyRef.current?.selectPillar(pillarIndex-1)}>↑</button>{pillars.map((pillar,index)=><a href={`#${pillar.route}`} key={pillar.id} aria-label={pillar.nav} aria-current={pillarIndex===index?"step":undefined} onClick={event=>{event.preventDefault();journeyRef.current?.selectPillar(index);window.history.replaceState(null,"",`#${pillar.route}`);}}><span>{pillar.letter}</span><span className="rail-title">{pillar.nav}</span></a>)}<button className="pillar-rail-arrow" aria-label="Próximo pilar" onClick={()=>journeyRef.current?.selectPillar(pillarIndex+1)}>↓</button></nav>}
